...
def test_average_age_under(people, maxage, output):
    assert ex6_hobby_counter.average_age_under(people, maxage) == output
...
def test_all_hobbies(people, output):
    assert ex6_hobby_counter.all_hobbies(people) == output
...
def test_hobby_counter(people, output):
    assert ex6_hobby_counter.hobby_counter(people) == output
...
def test_n_most_common(people, n, output):
    assert sorted(ex6_hobby_counter.n_most_common(people, 3)) == sorted(output)
