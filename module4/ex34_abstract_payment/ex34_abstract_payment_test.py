import pytest
from ex34_abstract_payment import AbstractPayment, CannotChargeError, CannotReverseError
...
def test_not_implement_authorize():
    ...
    with pytest.raises(TypeError) as e:
        mnp = MyNewPayment()
    assert 'authorize_payment' in e.value.args[0]
...
def test_not_implement_reverse():
    ...
    with pytest.raises(TypeError) as e:
        mnp = MyNewPayment()
    assert 'reverse_payment' in e.value.args[0]
...
def test_not_implement_charge():
    ...
    with pytest.raises(TypeError) as e:
        mnp = MyNewPayment()
    assert 'charge_payment' in e.value.args[0]
